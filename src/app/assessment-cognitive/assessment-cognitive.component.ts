...
@Component({
  selector: 'app-assessment-cognitive',
  templateUrl: './assessment-cognitive.component.html',
  styleUrls: ['./assessment-cognitive.component.css']
})
export class AssessmentCognitiveComponent implements OnDestroy{
...
//End Task Summary
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = 'a26f33fa-9ba0-7f83-b169-7fe4b10461ed';
   ...
   await this.subscriptions.add(
    ...
    .subscribe((response) => {
       ...
       //------------------------------------------------------------------------------------------------
        ...
        var cognitivecriteria = JSON.stringify(parsedResponse["reasonSForScreening"]);
        var cognitivedementiadiagnosis = parsedResponse["doesThePatientHaveAFormalDiagnosisOfDementia"];
        var cognitivecitscore = parsedResponse["citScore"];
        var cognitivecitguidance = parsedResponse["citGuidance"];
        var cognitiveformalcarer = parsedResponse["hasThePatientGotAFormalCarerOtherThanNextOfKin"];
        var cognitivepointofcontact = parsedResponse["whoIsTheFirstPointOfContactOtherThanNextOfKin"];
        var cognitiverequired = parsedResponse["doesThePatientRequireTheAssessment"];

        if(isNaN(cognitivecitscore)){
          cognitivecitscore = null;
        }
        ...
        this.subscriptions.add(
          ...
          .subscribe((summary) => {
            ...
            this.poaSummary.cognitivecriteria = cognitivecriteria;
            this.poaSummary.cognitivedementiadiagnosis = cognitivedementiadiagnosis;
            this.poaSummary.cognitivecitscore = cognitivecitscore;
            this.poaSummary.cognitivecitguidance = cognitivecitguidance;
            this.poaSummary.cognitiveformalcarer = cognitiveformalcarer;
            this.poaSummary.cognitivepointofcontact = cognitivepointofcontact;
            this.poaSummary.cognitiverequired = cognitiverequired;
            ...
            this.subscriptions.add(
              this.apiRequest.postRequest(this.postAsssessmentSummaryURI, this.poaSummary)
                .subscribe((response) => {
                  ...
                  if(cognitivecitscore >= 10) {

                    taskMessage = "Cognitive Screeing Tool >= 10, Refer to GP";
                    this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, "taskrefergp", "POA Nurse", "Please refer to GP", taskMessage);
                    this.poaTaskSummary.taskrefergp = true;
                  }

                  if(cognitivecitscore >= 8) {

                    taskMessage = "Cognitive Screeing Tool >= 8, Refer to Complex Case Team";
                    ...
                    taskMessage = "Cognitive Screeing Tool >= 8, Please insert delirium assessment proforma into skinny file";
                    this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, "taskinsertdeleriumproforma", "POA Nurse", "Please insert delirium assessment proforma into skinny file", taskMessage);
                    this.poaTaskSummary.taskinsertdeleriumproforma = true;
                  }

                  this.toasterService.showToaster("Success","Cognitive Screening Tool");
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Cognitive Assessment')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Cognitive Assessment','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
