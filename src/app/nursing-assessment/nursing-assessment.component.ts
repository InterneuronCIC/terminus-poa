...
@Component({
  ...
  styleUrls: ['./nursing-assessment.component.css']
})
export class NursingAssessmentComponent  implements OnDestroy{
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '6b80af6c-5651-0d23-6613-15800917274d';
   ...
   async markSectionCompleted(completed) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
        ...
        this.selectedPOA.iscompletednursingassessment = completed;
      ...
      .subscribe((response) => {
        ...
        .subscribe((data) => {
         ...
         if(this.appService.lockedOrBlocked) {
           console.log(this.appService.lockedOrBlocked);
          var resp = [];
  ...
  async onSubmit(submission: any) {
    ...
    //Prepare the required Variables
      ...
      var completedby = parsedResponse["completedByNurse"];
      var validatedBy = parsedResponse["revalidatedByNurse"];
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      this.RefreshPOA(completedby, validatedBy)
  ...
  async RefreshPOA(completedby: string, validatedby: string) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
       ...
       this.updatePOA.completedby = completedby;
       this.updatePOA.validatedby = validatedby;

        //update the entity record
        this.subscriptions.add(
          this.apiRequest.postRequest(this.postPOAURI, this.updatePOA)
            .subscribe((response) => {
              this.toasterService.showToaster("Success","Nursing Assessment Section Saved");

              if(this.appService.autoproceedtonextsection) {
                this.appService.viewToShow = 'print';
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Fitness Outcome')
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Fitness Outcome','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
...
async back() {
  ...
  this.appService.viewToShow = 'notes';
  this.viewClosed.emit(true);
}
