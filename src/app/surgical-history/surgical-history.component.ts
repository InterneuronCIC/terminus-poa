...
export class SurgicalHistoryComponent implements OnDestroy{
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '75618a12-b803-f8d5-9806-acf5331f2641';
   ...
   async markSectionCompleted(completed) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
        ...
        this.selectedPOA.iscompletedsurgicalhistory = completed;
  ...
  async onSubmit(submission: any) {
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      this.toasterService.showToaster("Success","Surgical History Section Saved");
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Surgical History')
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Surgical History','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
