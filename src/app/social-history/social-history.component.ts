...
export class SocialHistoryComponent implements OnDestroy{
...
//End Task Summary
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '7b754a96-0236-c147-b225-073beabef29b';
   ...
   async markSectionCompleted(completed) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
        ...
        this.selectedPOA.iscompletedsocialhistory = completed;
  ...
  sleep(ms) {
   ...
   buildDataObject() {
    ...
    //console.log("Building data object");
     ...
     //BMI from POA Baseline Observations
     //console.log(this.selectedPOA.bmi);
     if(this.selectedPOA.bmi > 25) {
      this.dataObject["data"]["bmiGreaterThan25"] = "yes";
     }
     else if(this.selectedPOA.bmi <= 25)
     {
      this.dataObject["data"]["bmiGreaterThan25"] = "no";
     }
     else {
  ...
  async onSubmit(submission: any) {
    ...
    var drugUseRecreationally = parsedResponse["drugUseRecreationally"];
    var alchoholSymptomsOfWithdrawl = parsedResponse["alchoholSymptomsOfWithdrawl"];

    var alcoholUnitsWeek = parsedResponse["alcoholUnitsWeek"];
    if(!Number(alcoholUnitsWeek)) {
      alcoholUnitsWeek = 0;
    }

    var alcoholConsumption = parsedResponse["alcoholConsumption"];

    var socialIsThePatientACarer = parsedResponse["socialIsThePatientACarer"];
    var socialIsThePatientKnownToSS = parsedResponse["socialIsThePatientKnownToSS"];
    var socialTypeOfHousing = parsedResponse["socialTypeOfHousing"]; // noFixedAbode

    var commsCommunication = parsedResponse["commsCommunication"]; // canCommunicateVerbally
    if(commsCommunication == "") {
      commsCommunication = "canCommunicateVerbally";
    }
   ...
   await this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      if(drugUseRecreationally  == 'yes') {
        taskMessage = "Patient uses drugs recreationally - Please refer to Acute Pain Team";
        taskName = "Please refer to Acute Pain Team";
        taskType = 'taskreferacutepainteam';
      ...
      if(alcoholUnitsWeek  >= 14 && alcoholConsumption != 'lessThan') {
        taskMessage = "Patient drinks more than 13 units of alcohol per week - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskrefercomplexcase = true;
      }
      else if (alcoholUnitsWeek  >= 13 && alcoholConsumption === 'moreThan') {
        taskMessage = "Patient drinks more than 13 units of alcohol per week - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
      ...
      if(alchoholSymptomsOfWithdrawl  == 'yes') {
        taskMessage = "Patient experiences symptoms of alcohol witdrawal - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskrefercomplexcase = true;
      ...
      if(socialIsThePatientACarer  == 'yes') {
        taskMessage = "Patient is a carer for someone - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskrefercomplexcase = true;
      ...
      if(socialIsThePatientKnownToSS  == 'yes') {
        taskMessage = "Patient is a known to Social Services - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskrefercomplexcase = true;
      ...
      if(socialTypeOfHousing  == 'noFixedAbode') {
        taskMessage = "Patient is of no fixed abode - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskrefercomplexcase = true;
      ...
      if(commsCommunication  != 'canCommunicateVerbally') {
        taskMessage = "Patient has trouble communicating - Please refer to Complex Case Team";
        taskName = "Please refer to Complex Case Team";
        taskType = 'taskrefercomplexcase';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskrefercomplexcase = true;
      ...
      if(socialTypeOfHousing  === 'nursingHome') {
        taskMessage = "Nursing Home - Inform Bed Management Team that patient requires side room";
        taskName = "Inform Bed Management Team that patient requires side room";
        taskType = 'taskbedmanagementsideroom';
        this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
        this.poaTaskSummary.taskbedmanagementsideroom = true;
      }


      this.toasterService.showToaster("Success","Social History Section Saved");

      if(this.appService.autoproceedtonextsection) {
        this.appService.viewToShow = 'assessments';
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Social History')
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Social History','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
...
async back() {
  ...
  this.appService.viewToShow = 'familyHistory';
  this.viewClosed.emit(true);
}
