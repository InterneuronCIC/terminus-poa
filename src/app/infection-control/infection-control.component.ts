...
export class InfectionControlComponent implements OnDestroy{
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '3d5250e0-26ac-2fe1-95e5-cb9e291cc67b';
   ...
   async markSectionCompleted(completed) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
        ...
        this.selectedPOA.iscompletedinfectioncontrol = completed;
  ...
  async onSubmit(submission: any) {
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      this.toasterService.showToaster("Success","Infection Control Section Saved");
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Infection Control')
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Infection Control','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
