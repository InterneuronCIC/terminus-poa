...
@Component({
  selector: 'app-assessment-aki',
  templateUrl: './assessment-aki.component.html',
  styleUrls: ['./assessment-aki.component.css']
})


export class AssessmentAKIComponent implements OnDestroy{
...
//End Task Summary
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '55c76d36-16c5-fd43-7c1f-ab53b04e2c84';
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      //------------------------------------------------------------------------------------------------
        ...
        var akiAssessmentStatus = parsedResponse["akiAssessmentStatus"]
        var akiGuidance = parsedResponse["sortAssessmentNotes"]
        var predictedAkiRisk = parsedResponse["predictedAkiRisk"];
        var akiadditionalriskfactors = parsedResponse["additionalRiskFactorsCount"];


        akiAssessmentStatus = akiAssessmentStatus.toUpperCase();
        if(akiAssessmentStatus == "NOTREQUIRED") {
          akiAssessmentStatus = "NOT REQUIRED"
        }

        if(isNaN(predictedAkiRisk)){
          predictedAkiRisk = null;
          akiGuidance = null;
        }
        else if(predictedAkiRisk > 10) {
          akiGuidance = "High Risk AKI Patient - AKI Assessment Score is > 10%"
        }
        else if(predictedAkiRisk <= 10 && !isNaN(akiadditionalriskfactors) && akiadditionalriskfactors > 4) {
          akiGuidance = "High Risk AKI Patient - AKI Assessment Score is <= 10% but additional risk factors > 4"
        }
        else if(predictedAkiRisk <= 10 && !isNaN(akiadditionalriskfactors) && akiadditionalriskfactors <= 4) {
          akiGuidance = "No further action needed - AKI Assessment Score is <= 10% and additional risk factors <= 4"
        }
        else {
          akiGuidance = null;
        }
        if(isNaN(akiadditionalriskfactors)){
          akiadditionalriskfactors = null;
        }
      ...
      var taskType = '';
        ...
        this.subscriptions.add(
          ...
          .subscribe((summary) => {
            ...
            this.poaSummary.akistatus = akiAssessmentStatus;
            this.poaSummary.akirisk = predictedAkiRisk;
            this.poaSummary.akiadditionalriskfactors = akiadditionalriskfactors;
            this.poaSummary.akiguidance = akiGuidance;
            //update the entity record
            this.subscriptions.add(
              this.apiRequest.postRequest(this.postAsssessmentSummaryURI, this.poaSummary)
                .subscribe((response) => {

                  this.toasterService.showToaster("Success","AKI Assessment Saved");

                  if(akiGuidance) {
                    if(akiGuidance.includes('High Risk')) {


                      taskMessage = "AKI Risk is High - Provide patient leaflet and ensure AKI Risk document inserted into skinny file";
                      taskName = "Provide patient leaflet and ensure AKI Risk document inserted into skinny file";
                      taskType = 'taskinsertdeleriumproforma';
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'AKI Assessment')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'AKI Assessment','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
