...
@Component({
  selector: 'app-information-provided',
  templateUrl: './information-provided.component.html',
  styleUrls: ['./information-provided.component.css']
})
export class InformationProvidedComponent implements OnDestroy{
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = 'd9db67da-7cff-19b8-79ca-7f71f874284e';
   ...
   async markSectionCompleted(completed) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
        ...
        this.selectedPOA.iscompletedinformationprovided = completed;
  ...
  async onSubmit(submission: any) {
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      this.toasterService.showToaster("Success","Information Provided Section Saved");
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Information Provided')
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Information Provided','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
