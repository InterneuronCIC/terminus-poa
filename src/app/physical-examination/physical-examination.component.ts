...
export class PhysicalExaminationComponent implements OnDestroy{
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = 'abc10bfa-e0e4-8ea6-9c63-721504302616';
   ...
   async markSectionCompleted(completed) {
    ...
    this.subscriptions.add(
      ...
      .subscribe((response) => {
        ...
        this.selectedPOA.iscompletedphysicalexamination = completed;
  ...
  async onSubmit(submission: any) {
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      this.toasterService.showToaster("Success","Physical Examination Section Saved");
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Physical Examination')
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Physical Examination','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
