...
@Component({
  selector: 'app-assessment-adult-malnutrition',
  templateUrl: './assessment-adult-malnutrition.component.html',
  styleUrls: ['./assessment-adult-malnutrition.component.css']
})
export class AssessmentAdultMalnutritionComponent implements OnDestroy{
 ...
 //End Task Summary
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '34f85521-2931-60d7-da5a-19e8478e1a7c';
   ...
   this.subscriptions.add(
    ...
    .subscribe((response) => {
      ...
      //------------------------------------------------------------------------------------------------
        ...
        var malnutritionscore = parsedResponse["totalScore"];
        var malnutritionguidance = parsedResponse["guidance"]


        if(isNaN(malnutritionscore)){
          malnutritionscore = null;
        }
        ...
        this.subscriptions.add(
          ...
          .subscribe((summary) => {
            ...
            this.poaSummary.malnutritionscore = malnutritionscore;
            this.poaSummary.malnutritionguidance = malnutritionguidance;
            ...
            this.subscriptions.add(
              this.apiRequest.postRequest(this.postAsssessmentSummaryURI, this.poaSummary)
                .subscribe((response) => {
                  ...
                  if(malnutritionscore >= 3 && malnutritionscore <= 6) {
                    taskMessage = "Adult Malnutrition Assessment - High Risk, Refer to Dietetic Team (Urgent)";
                    this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, "taskreferdieteticteam", "POA Nurse", "Please refer to Dietetic Team", taskMessage);
                    this.poaTaskSummary.taskreferdieteticteam = true;
                  }
                  else if(malnutritionscore >= 1 && malnutritionscore <= 2) {
                    taskMessage = "Adult Malnutrition Assessment - Medium Risk, Refer to Dietetic Team";
                    this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, "taskreferdieteticteam", "POA Nurse", "Please refer to Dietetic Team", taskMessage);
                    this.poaTaskSummary.taskreferdieteticteam = true;
                  }
                  this.toasterService.showToaster("Success","Adult Malnutrition Assessment Saved");
                  this.assessmentsViewClosed.emit(true);
                })
              )
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'Adult Malnutrition Assessment')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'Adult Malnutrition Assessment','','Import')
    .then((confirmed) => response = confirmed)
    .catch(() => response = false);
    if(!response) {
      return;
