...
@Component({
  selector: 'app-assessment-sort',
  templateUrl: './assessment-sort.component.html',
  styleUrls: ['./assessment-sort.component.css']
})
export class AssessmentSORTComponent implements OnDestroy {
  ...
  poaId: string;
  personId: string;
  formBuilderFormId: string = '6a295e6f-eb13-046d-8f1c-b42a2e200221';
    ...
    await this.subscriptions.add(
      this.apiRequest.postRequest(this.appService.baseURI + '/PostObject?synapsenamespace=core&synapseentityname=formbuilderresponse', this.formResponse)
        .subscribe((response) => {
          ...
          var sortAssessmentStatus = parsedResponse["sortAssessmentStatus"]
          var sortAssessmentNotes = parsedResponse["sortAssessmentNotes"]
          var estimatedRiskOf30DayMortality = parsedResponse["estimatedRiskOf30DayMortality"];

          var taskMessage = '';
          var requiresHDU = false;

          sortAssessmentStatus = sortAssessmentStatus.toUpperCase();
          if (sortAssessmentStatus == "NOTREQUIRED") {
            sortAssessmentStatus = "NOT REQUIRED"
          }
          if (isNaN(estimatedRiskOf30DayMortality)) {
            estimatedRiskOf30DayMortality = null;
          }


          //alert (estimatedRiskOf30DayMortality);
          if (estimatedRiskOf30DayMortality > 5) {
            requiresHDU = true;
            taskMessage = "Task Created (HDU) - Refer to anaesthetist, patient may require HDU";
          }
          ...
          this.subscriptions.add(
            this.apiRequest.getRequest(this.getAsssessmentSummaryURI)
              .subscribe((summary) => {
                ...
                this.poaSummary.sortstatus = sortAssessmentStatus;
                this.poaSummary.sortrisk = estimatedRiskOf30DayMortality;
                this.poaSummary.sortnotes = sortAssessmentNotes;
                ...
                this.subscriptions.add(
                  this.apiRequest.postRequest(this.postAsssessmentSummaryURI, this.poaSummary)
                    .subscribe((response) => {
                      this.toasterService.showToaster("Success", "SORT Assessment Saved");

                      this.RefreshPOA(requiresHDU, taskMessage);
          ...
          this.subscriptions.add(
            this.apiRequest.postRequest(this.postPOAURI, this.updatePOA)
              .subscribe((response) => {
                ...
                // if(requiresHDU) {
                //   requiresHDU = true;
                //   this.taskAutomationService.checkTask(this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, "taskreferanaesthetist", "POA Nurse", "Please refer to anaesthetist", "Patient requires HDU Post-Operatively (SORT score – if > 5%), please refer to anaesthetist");
                // }
                ...
                if (requiresHDU) {
                  ...
                  taskMessage = "SORT score > 5% - Consider referring to anaesthetist";
                  ...
                  taskMessage = "SORT score > 5% - Consider booking an HDU bed";
                  taskName = "Consider booking an HDU bed";
                  taskType = 'taskbookhdubed';
                  this.taskAutomationService.checkTask(this.poaTaskSummary, this.selectedPOA.poa_preopassessment_id, this.selectedPOA.person_id, taskType, "POA Nurse", taskName, taskMessage);
  ...
  async viewHistory() {
    var response = false;
    await this.formioHistoryService.confirm(this.formContextKey, 'SORT Assessment')
      .then((confirmed) => response = confirmed)
      .catch(() => response = false);
    if (!response) {
      return;
  ...
  async viewPrevious() {
    var response = false;
    await this.formioImportService.confirm(this.formBuilderFormId, this.personId, this.formResponse, 'SORT Assessment', '', 'Import')
      .then((confirmed) => response = confirmed)
      .catch(() => response = false);
    if (!response) {
      return;
